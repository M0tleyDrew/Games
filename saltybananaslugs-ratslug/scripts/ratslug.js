...
function freshState() {
  return {
    phase:"lobby", paused:false, players:[], personas:{},
    promptDeck:[...RATSLUG_PROMPTS], usedPrompts:[], currentPrompt:null,
    responses:{}, messages:[], guesses:{}, favorites:{}, revealed:[],
    round:1, promptNumber:0, roundPrompts:[], roundSubmissions:{},
    scores:{}, favoriteTotals:{}, roundResults:[], pendingRevealCandidates:[], winnerIds:[]
  };
}
...
const responseSignal = () => `${state.round}:` + (state.roundPrompts || [])
  .map(item => `${item.number}:${Object.keys(item.responses || {}).sort().join(",")}`)
  .join("|");

function draftKey(element, round=state.round) {
  const field = element.id || (element.dataset.guess ? `guess-${element.dataset.guess}` : null);
  return field ? `${activeTab}:${round}:${field}` : null;
}

function captureDraft(root, round=state.round) {
  if (!root) return;
  root.querySelectorAll("input[id],textarea[id],select[id],[data-guess]").forEach(element => {
    if (element.type === "file") return;
    const key = draftKey(element, round);
    if (key) uiDraft[key] = element.value;
  });
}
...
function restoreDraft(root) {
  if (!root) return;
  root.querySelectorAll("input[id],textarea[id],select[id],[data-guess]").forEach(element => {
    if (element.type === "file" || element.disabled) return;
    const key = draftKey(element);
    if (key && Object.prototype.hasOwnProperty.call(uiDraft, key)) element.value = uiDraft[key];
  });
  if (pendingPersonaImage && !state.personas[game.user.id]) {
    const preview = root.querySelector("#rs-image-preview");
    if (preview) preview.outerHTML = `<img id="rs-image-preview" src="${pendingPersonaImage}">`;
  }
}

function viewKey(round=state.round) { return `${activeTab}:${round}`; }

function captureView(root, round=state.round) {
  if (!root) return;
  captureDraft(root, round);
  const main = root.querySelector("main");
  const focused = root.contains(document.activeElement) ? document.activeElement : null;
  const key = focused ? draftKey(focused, round) : null;
  uiView[viewKey(round)] = {
    mainScroll: main?.scrollTop || 0,
    scrolls: Object.fromEntries([...root.querySelectorAll("[data-rs-scroll]")].map(element => [element.dataset.rsScroll, element.scrollTop])),
    focus:key,
...
function restoreView(root) {
  if (!root) return;
  restoreDraft(root);
  const saved = uiView[viewKey()];
  if (!saved) return;
  const main = root.querySelector("main");
  if (main) main.scrollTop = saved.mainScroll || 0;
  root.querySelectorAll("[data-rs-scroll]").forEach(element => {
    if (Object.prototype.hasOwnProperty.call(saved.scrolls || {}, element.dataset.rsScroll)) {
      element.scrollTop = saved.scrolls[element.dataset.rsScroll];
    }
  });
  if (saved.focus) {
    const focused = [...root.querySelectorAll("input[id],textarea[id],select[id],[data-guess]")]
      .find(element => draftKey(element) === saved.focus && !element.disabled);
    if (focused) {
      focused.focus({preventScroll:true});
      if (saved.selectionStart !== null && typeof focused.setSelectionRange === "function") {
...
Hooks.once("ready", async () => {
  ...
  Hooks.on("renderPlayerList", (_app, html) => {
    ...
    b.className="ratslug-launch";
    b.innerHTML='<i class="fas fa-rat"></i> RatSlug';
    b.onclick=openUI;
    root.append(b);
  });
  ...
  if (!document.querySelector(".ratslug-fab")) {
    ...
    fab.className="ratslug-fab";
    fab.innerHTML='<i class="fas fa-rat"></i><span>RatSlug</span>';
    fab.onclick=openUI;
...
function receive(data) {
  if (data.type === "state") {
    const previousPhase=state.phase, previousRound=state.round;
    captureView(document.querySelector(".ratslug-shell"), previousRound);
    if (data.state.phase === "lobby" && previousPhase !== "lobby") { uiDraft={}; uiView={}; }
    state=data.state;
    state.personas ??= {};
    skipNextDraftCapture=true;
    if (!game.user.isGM && state.phase === "voting" && previousPhase !== "voting") activeTab="guess";
    if (!game.user.isGM && state.phase === "play" && previousPhase === "roundResult") activeTab="prompts";
    if (!game.user.isGM && ["roundResult","revealChoice","end"].includes(state.phase) && state.phase !== previousPhase) activeTab="results";
    render();
    return;
  }
...
function correctGuesses(s, playerId) {
  const guesses=s.roundSubmissions[s.round]?.[playerId]?.guesses || {};
  return s.players.filter(id => id !== playerId && guesses[id] === id).length;
}

function finishReveal(s, playerId) {
  if (playerId && !s.revealed.includes(playerId)) s.revealed.push(playerId);
  const result=s.roundResults.at(-1);
  if (result) result.revealedId=playerId || null;
  s.pendingRevealCandidates=[];
  const survivors=s.players.filter(id => !s.revealed.includes(id));
  if (survivors.length <= 1) { s.winnerIds=survivors; s.phase="end"; }
  else s.phase="roundResult";
}

function scoreRound(s) {
  const earned={};
  for (const id of s.players) {
    earned[id]=correctGuesses(s,id);
    s.scores[id]=(s.scores[id] || 0) + earned[id];
    const favorite=s.roundSubmissions[s.round]?.[id]?.favorite;
    if (favorite) s.favoriteTotals[favorite]=(s.favoriteTotals[favorite] || 0) + 1;
  }
  ...
  const lowTotal=Math.min(...active.map(id => s.scores[id] || 0));
  let candidates=active.filter(id => (s.scores[id] || 0) === lowTotal);
  const lowRound=Math.min(...candidates.map(id => earned[id]));
  candidates=candidates.filter(id => earned[id] === lowRound);
  if (candidates.length === 1) finishReveal(s,candidates[0]);
  else {
    s.pendingRevealCandidates=candidates;
    s.roundResults.at(-1).revealCandidates=[...candidates];
    s.phase="revealChoice";
  }
}
...
function handle(action, userId, p={}) {
  ...
  if (action === "persona" && !s.personas[userId] && String(p.name || "").trim()) {
    s.personas[userId]={
  ...
  if (action === "message" && !s.paused && s.players.includes(userId) && p.text?.trim()) {
    s.messages.push({id:uid(), userId, recipientId:p.recipientId || null, text:String(p.text).slice(0,500), at:Date.now()});
  }

  if (action === "response" && !s.paused && s.phase === "play" && s.players.includes(userId) && String(p.text || "").trim()) {
    const target=s.roundPrompts.find(x => x.number === Number(p.promptNumber));
    if (target && !Object.prototype.hasOwnProperty.call(target.responses || {}, userId)) {
      target.responses ??= {};
      target.responses[userId]=String(p.text || "").slice(0,1000);
      if (target.number === s.promptNumber) s.responses=foundry.utils.deepClone(target.responses);
    }
  }

  if (action === "submitRound" && s.phase === "voting" && s.players.includes(userId) && !s.roundSubmissions[s.round]?.[userId]) {
    const others=s.players.filter(id => id !== userId), guesses=p.guesses || {}, values=Object.values(guesses);
    if (others.every(id => others.includes(guesses[id])) && values.length === others.length && new Set(values).size === values.length && others.includes(p.favorite)) {
      s.roundSubmissions[s.round] ??= {};
      s.roundSubmissions[s.round][userId]={guesses, favorite:p.favorite, lockedAt:Date.now()};
    }
  }

  if (isGM && action === "start" && s.players.length > 0 && s.players.every(id => s.personas[id])) {
    s.phase="play"; s.round=1; s.promptNumber=0; s.currentPrompt=null; s.responses={}; s.roundPrompts=[];
  }
  if (isGM && action === "prompt" && s.phase === "play" && s.promptNumber < 4) {
    if (s.currentPrompt) s.usedPrompts.push(s.currentPrompt);
    s.currentPrompt=String(p.text || "").slice(0,500);
    s.promptDeck=s.promptDeck.filter(x => x !== s.currentPrompt);
    s.responses={}; s.promptNumber++;
    s.roundPrompts.push({round:s.round, number:s.promptNumber, text:s.currentPrompt, responses:{}});
  }
  if (isGM && action === "customPrompt" && s.phase === "play" && s.promptNumber < 4) {
    if (s.currentPrompt) s.usedPrompts.push(s.currentPrompt);
    s.currentPrompt=String(p.text || "").slice(0,500);
    s.responses={}; s.promptNumber++;
    s.roundPrompts.push({round:s.round, number:s.promptNumber, text:s.currentPrompt, responses:{}});
  ...
  if (isGM && action === "clearPrompt" && s.phase === "play" && s.currentPrompt) {
    s.usedPrompts.push(s.currentPrompt);
    s.roundPrompts=s.roundPrompts.filter(x => x.number !== s.promptNumber);
    s.promptNumber=Math.max(0,s.promptNumber-1); s.currentPrompt=null; s.responses={};
  }
...
function openUI() {
  document.querySelector(".ratslug-shell")?.remove();
  const el=document.createElement("div");
  el.className=`ratslug-shell ${game.user.isGM ? 'ratslug-host' : 'ratslug-player'}`;
  el.addEventListener("click", e => { if (e.target.closest("[data-pause]")) act("togglePause"); });
...
function metaLine(p) {
  const bits=[];
  if (p.age) bits.push(`Age ${esc(p.age)}`);
  if (p.pronouns) bits.push(esc(p.pronouns));
  if (p.location) bits.push(esc(p.location));
  return bits.join(" · ");
}

function currentView() {
  const root=document.querySelector(".ratslug-shell");
  if (!skipNextDraftCapture) captureView(root);
  skipNextDraftCapture=false;
  ...
  if (activeTab === "guess") return guess();
  if (activeTab === "host" && game.user.isGM) return host();
  return home();
}
...
function render() {
  ...
  const anyResponses=state.roundPrompts.some(item => Object.keys(item.responses || {}).length);
  if (activeTab === "prompts") seenResponseSignal=signal;
  const hasNewResponses=anyResponses && signal !== seenResponseSignal;
  const showResults=["roundResult","revealChoice","end"].includes(state.phase);
  const showProfiles=state.players.some(id => state.personas[id]);
  root.innerHTML=`<header><img src="modules/${ID}/assets/ratslug.png"><div><h1>RatSlug</h1><small>Round ${state.round} · Prompt ${state.promptNumber}/4 · ${esc(state.phase)}${state.paused ? ' · PAUSED' : ''}</small></div><button data-x>×</button></header><nav><button data-tab="home" class="${activeTab==='home'?'active':''}">Game</button>${showProfiles ? `<button data-tab="profiles" class="${activeTab==='profiles'?'active':''}">Profiles</button>` : ''}<button data-tab="prompts" class="${activeTab==='prompts'?'active ':''}${hasNewResponses?'has-responses':''}">Prompts</button><button data-tab="chat" class="${activeTab==='chat'?'active':''}">Chat</button><button data-tab="guess" class="${activeTab==='guess'?'active':''}">Identity Board</button>${showResults ? `<button data-tab="results" class="${activeTab==='results'?'active':''}">Results</button>` : ''}${game.user.isGM ? `<button data-tab="host" class="${activeTab==='host'?'active':''}">Host</button>` : ''}</nav><main>${state.paused ? '<div class="paused-banner">The host has paused the game.</div>' : ''}${currentView()}</main>`;
  bind(root);
}
...
function bind(root) {
  ...
  root.querySelectorAll("input[id],textarea[id],select[id],[data-guess]").forEach(element => {
    if (element.type !== "file") {
      const remember=() => { const key=draftKey(element); if (key) uiDraft[key]=element.value; };
  ...
  if(q("[data-save-persona]")) q("[data-save-persona]").onclick=() => {
    const name=q("#rs-name").value.trim();
    if(!name) return ui.notifications.warn("Your persona needs a name before it can be locked.");
    act("persona",{
      name,
      age:q("#rs-age")?.value || "",
    ...
    pendingPersonaImage=null;
  };
  if(q("#rs-image")) q("#rs-image").onchange=e => compress(e.target.files[0]).then(image => {
    pendingPersonaImage=image;
    const preview=q("#rs-image-preview");
    if(preview) preview.outerHTML=`<img id="rs-image-preview" src="${image}">`;
  });
  root.querySelectorAll("[data-response]").forEach(button => button.onclick=() => {
    const promptNumber=Number(button.dataset.response), input=q(`#rs-response-${promptNumber}`);
    if(input?.value.trim()) act("response",{promptNumber,text:input.value});
  });
  if(q("[data-send]")) q("[data-send]").onclick=() => {
    const input=q("#rs-chat"), text=input.value, recipientId=q("#rs-recipient")?.value || null;
    if(!text.trim()) return;
    input.value="";
    act("message",{text,recipientId});
  };
  if(q("[data-submit-round]")) q("[data-submit-round]").onclick=() => {
    const guesses=Object.fromEntries([...root.querySelectorAll("[data-guess]")].map(x => [x.dataset.guess,x.value]));
    const values=Object.values(guesses), favorite=q("#rs-favorite").value;
    if(values.some(v => !v) || new Set(values).size !== values.length) return ui.notifications.warn("Match every persona and use each real player exactly once.");
    if(!favorite) return ui.notifications.warn("Choose a Favorite Character before locking the round.");
    act("submitRound",{guesses,favorite});
  };
  if(q("[data-random]")) q("[data-random]").onclick=() => {
    if(!state.promptDeck.length) return ui.notifications.warn("The prompt deck is empty.");
    act("prompt",{text:state.promptDeck[Math.floor(Math.random()*state.promptDeck.length)]});
  };
...
async function compress(file) {
  ...
  const max=512, scale=Math.min(1,max/Math.max(bitmap.width,bitmap.height));
  const c=document.createElement("canvas");
  c.width=Math.round(bitmap.width*scale); c.height=Math.round(bitmap.height*scale);
  c.getContext("2d").drawImage(bitmap,0,0,c.width,c.height);
  return c.toDataURL("image/webp",.78);
}
